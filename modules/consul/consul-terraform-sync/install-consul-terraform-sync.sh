...
function print_usage {
  echo
  echo "Usage: install-consul-terraform-sync [OPTIONS]"
  echo
  ...
  echo "Example:"
  echo
  echo "  install-consul-terraform-sync --version 1.2.2"
}
...
function install_binary {
  ...
  local -r consul_terraform_sync_dest_path="$bin_dir/"

  unzip -d /tmp "$DOWNLOAD_PACKAGE_PATH"

  log_info "Moving Consul-Terraform-Sync binary to $consul_terraform_sync_dest_path"
  sudo mv "/tmp/consul-terraform-sync" "$consul_terraform_sync_dest_path"
...
function install {
  ...
  # on the directories where Consul-Terraform-Sync may be installed. See https://github.com/hashicorp/terraform-aws-consul/issues/210.
  if sudo -E PATH=$PATH bash -c "command -v consul-terraform-sync"; then
