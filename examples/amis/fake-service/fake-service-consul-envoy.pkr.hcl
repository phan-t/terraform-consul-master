...
source "amazon-ebs" "ubuntu20-ami" {
  ...
  tags = {
    ...
    packer_source   = "https://github.com/phan-t/terraform-consul-master/blob/master/examples/amis/fake-service/fake-service-consul.pkr.hcl"
  }
}
...
build {
  ...
  provisioner "shell" {
    inline       = ["if test -n \"${var.fake_service_download_url}\"; then", "/tmp/terraform-consul-master/examples/amis/fake-service/scripts/install-fake-service --download-url ${var.fake_service_download_url};", "else", "/tmp/terraform-consul-master/examples/amis/fake-service/scripts/install-fake-service --version ${var.fake_service_version};", "fi"]
    pause_before = "30s"
  }
}